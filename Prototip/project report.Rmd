...
#### For time series data, the obvious graph to start with is a time plot. That is, the observations are plotted against the time of observation, with consecutive observations joined by straight lines. Figure 2.1 below shows the weekly economy passenger load on Ansett Airlines between Australia‚Äôs two largest cities.
...
#### A trend exists when there is a long-term increase or decrease in the data. It does not have to be linear. Sometimes we will refer to a trend as ‚Äúchanging direction‚Äù, when it might go from an increasing trend to a decreasing trend. There is a trend in the antidiabetic drug sales data shown in Figure 2.2.
...
#### A cycle occurs when the data exhibit rises and falls that are not of a fixed frequency. These fluctuations are usually due to economic conditions, and are often related to the ‚Äúbusiness cycle‚Äù. The duration of these fluctuations is usually at least 2 years.
...
#### 1.The monthly housing sales (top left) show strong seasonality within each year, as well as some strong cyclic behaviour with a period of about 6‚Äì10 years. There is no apparent trend in the data over this period.
...
#### A seasonal plot is similar to a time plot except that the data are plotted against the individual ‚Äúseasons‚Äù in which the data were observed. An example is given below showing the antidiabetic drug sales.
...
<center>
![](correlationFormula.png)
<center>


#### The value of **r** always lies between **‚?í1** and **1** with negative values indicating a negative relationship and positive values indicating a positive relationship. The following graphs show examples of data sets with varying levels of correlation.
...
#### The next figure displays scatterplots of quarterly Australian beer production, where the horizontal axis shows lagged values of the time series. Each graph shows **yt** plotted against **yt‚?ík** for different values of k.
...
#### The slow decrease in the ACF as the lags increase is due to the trend, while the ‚Äúscalloped‚Äù shape is due the seasonality.
...
#### For white noise series, we expect each autocorrelation to be close to zero. Of course, they will not be exactly equal to zero as there is some random variation. For a white noise series, we expect 95% of the spikes in the ACF to lie within ¬±2/‚?öT where T is the length of the time series.
 
#### In this example, T=50 and so the bounds are at ¬±2/‚?ö50=¬±0.28.All of the autocorrelation coefficients lie within these limits, confirming that the data are white noise.
